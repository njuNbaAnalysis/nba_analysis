...
public class MatchTablePanel extends JPanel {
	...
	private int width;
	private int height;
	private static String [] columnName = {"姓名","位置","分钟","%","命中","出手","三分%","三分命中","三分出手","罚球%","罚球命中","罚球出手","+/-","进攻","防守","篮板","助攻","犯规","抢断","失误","盖帽","得分"};
	...
	public MatchTablePanel(int width, int height, Match match, BLService bl) {
		...
		MatchJTable firstTeamRecord = new MatchJTable(match.getFirstRecordList());
		...
		MatchJTable secondTeamRecord = new MatchJTable(match.getSecondRecordList());
	...
	public void paintComponent(Graphics g) {
		...
		Image teamImage1 =  bl.getTeamByName(match.getTeams()[0]).getLogo(width/20, height / 20);
	...
	private class MatchJTable extends BaseJTable {
		...
		private String[] getIndividualDataRow(RecordOfPlayer recordOfPlayer) {
			...
			data[2] = Integer.toString(recordOfPlayer.getMinutes());
			data[3] = Double.toString(recordOfPlayer.getFieldGoalPercentage());
			data[4] = Integer.toString(recordOfPlayer.getFieldGoalHits());
			data[5] = Integer.toString(recordOfPlayer.getFieldGoalAttempts());			
			data[6] = Double.toString(recordOfPlayer.getThreePointPercentage());
			data[7] = Integer.toString(recordOfPlayer.getThreePointHits());
			data[8] = Integer.toString(recordOfPlayer.getThreePointAttemps());
			data[9] = Double.toString(recordOfPlayer.getFreeThrowPercentage());
	...
	public static void main(String [] args){
		...
		MatchTablePanel m = new MatchTablePanel(1280,800,bl.getAllMatches().get(0),bl);
