...
public class TeamComparePanel extends JPanel {
	...
	private int width;
	private int height;
	private int defauleNum = 11;// 默认显示的选项数量
	...
	TeamComparePanel(int width, int height, String[] attributeNames, Match m) {
		...
		CompareBarChartPanel chartPanel = new CompareBarChartPanel(width,
				height * 9 / 10, attribute, attributeNames, getTeamValue(m),
				getRivalValue(m), cof);
		chartPanel.setLocation(0, height / 20);
		this.add(chartPanel);
		repaint();
	}
	...
	public void loadImage() {
		...
		try {
			...
			bufferTable = ImageIO.read(new File("image" + File.separator
					+ "setting.png"));
			bufferTable = UIUtils.resize(bufferTable, width / 30, height / 20+1);
	...
	public void paintComponent(Graphics g) {
		...
		g2.fillRect(0, 0, width * 29 / 30+1, height / 20);
		// 球队对比
		...
		g2.drawString("球队对比", width / 2 - strWidth / 2, height / 40 +strHeight/4);
	}
			...
			for (int i = 0; i < attr_name.length; i++) {
				if (attr[i]) {
					...
					g2.drawString(a_value[i] + "", half - leftBarWidth - width
							/ 20, (int) (barHeight * (i + 1.5)
							+ barHeightSeparator * (i + 1) - strHeight / 2));

					g2.setColor(a);
					g2.fillRect(half - leftBarWidth, (int) (barHeight * (i+0.5)
							+ barHeightSeparator * (i + 1)), leftBarWidth,
							barHeight);

					// 右长方形
					int rightBarWidth = (int) (b_value[i] / coefficient[i] * half);
					g2.setColor(b);
					g2.fillRect(width - half, (int) (barHeight * (i+0.5)
							+ barHeightSeparator * (i + 1)), rightBarWidth,
							barHeight);

					g2.setFont(new Font("微软雅黑", Font.PLAIN, height / 30));
					g2.setColor(Color.black);
					g2.drawString(b_value[i] + "",
							width - half + rightBarWidth+width/20,
							(int) (barHeight * (i + 1.5) + barHeightSeparator
									* (i + 1) - strHeight / 2));
