    ...
    private void computeData(){
        ...
        for(int i=0;i<ListOfMatches.size();i++){
        	...
        	for(int j=0;j<ListOfPlayers1.size();j++){
        		UpdataPlayer(ListOfPlayers1.get(j), ListOfMatches.get(i).getTeams()[0]);
        	}
        	for(int j=0;j<ListOfPlayers2.size();j++){
        		UpdataPlayer(ListOfPlayers2.get(j), ListOfMatches.get(i).getTeams()[1]);
        	}
        }
    ...
    private void UpdataPlayer(RecordOfPlayer record,String team){
    	for(int i=0;i<playerList.size();i++){
    		if(playerList.get(i).getName().equals(record.getPlayerName())){
    			...
    			temp.setDefenseRebounds(temp.getDefenseRebounds()+record.getDefensiveRebounds());
    			temp.setFieldGoalAttempts(temp.getFieldGoalAttempts()+record.getFieldAttempts());
    			temp.setFieldGoalHits(temp.getFieldGoalHits()+record.getFieldGoals());
    			temp.setFouls(temp.getFouls()+record.getFauls());
    			temp.setFreeThrowAttempts(temp.getFieldGoalAttempts()+record.getThreePointAttemps());
    			temp.setFreeThrowHits(temp.getFreeThrowHits()+record.getFreeThrows());
    			temp.setMinutes(temp.getMinutes()+record.getMinutes());
    			temp.setOffenseRebounds(temp.getOffenseRebounds()+record.getOffensiveRebounds());
    			...
    			temp.setTeam(team);
    			temp.setThreePointerAttempts(temp.getThreePointerAttempts()+record.getThreePointAttemps());
    			temp.setThreePointerHits(temp.getThreePointerHits()+record.getThreePoints());
